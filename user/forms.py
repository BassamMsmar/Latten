...
class FormRegisterUser(UserCreationForm):
    ...
    fristname = forms.CharField(max_length=150, required=False, label='الاسم الأول ')
    lastname = forms.CharField(max_length=150, required=False, label='الاسم الأخير')
    email = forms.EmailField(max_length=254, label='البريد الالكتروني ')

    class Meta:
        model = User
        fields = (
            'username', 'password1', 'password2', 'email', 'fristname', 'lastname'
        )